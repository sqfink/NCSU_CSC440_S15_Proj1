...
public class StudentDisplayPastInvoiceState extends State {
	...
	public String doState(Runner r) {
		...
		try {
			List<InvoiceBean> invoices = Dao.getPaidInvoices(user.snumber);
			if (invoices == null || invoices.size() == 0) {
				System.out.println("No past invoices");
		...
		for (InvoiceBean b : invoiceData.keySet()) {
			...
			System.out.println("Payment received by staff ID " + b.staffnumber + " on " + b.paiddate + " as " + b.paymenttype);
		}
		return StudentViewInvoicesState.class.getName();
	}
